	...
	void Update(){
		...
		if(meleefwd){
			...
			if(GetComponent<Rigidbody2D>().gravityScale == 0){
				...
				GetComponent<Rigidbody2D>().linearVelocity = dir * 4;
			}else{
				Vector3 dir = attackPoint.TransformDirection(Vector3.right);
		...
		if(stat.flinch){
			cancelAttack = true;
			GetComponent<Rigidbody2D>().linearVelocity = stat.knock * stat.knockForce;
			return;
		}
		...
		if(onMoving){
			...
			GetComponent<Rigidbody2D>().linearVelocity = movDir * movSpd;
		}
		...
		if(followState == AIState.Moving){
			...
			if(distance <= approachDistance) {
				...
				if(GetComponent<Rigidbody2D>().gravityScale > 0){
					GetComponent<Rigidbody2D>().linearVelocity = transform.TransformDirection(Vector3.right) * speed;
				}else{
					if(playerPointer){
						...
						GetComponent<Rigidbody2D>().linearVelocity = playerPointer.TransformDirection(Vector3.right) * speed;
					}else{
						...
						GetComponent<Rigidbody2D>().linearVelocity = -destination;
					}
				}
			...
			if(patrolSetting.enable){
				if(patrolState == 1){//Moving
					GetComponent<Rigidbody2D>().linearVelocity = pfwd * patrolSetting.patrolSpeed;
				}
				//----------------------------
